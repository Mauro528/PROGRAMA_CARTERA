...
public class app extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        mainPanel.setOnKeyPressed(event -> {
            String frameActivo = "" + layout.getChildren().get(0);

            if (event.getCode() == KeyCode.RIGHT) {
                if (frameActivo.contains("Informes")) {
                    layout.getChildren().clear();
                    windowsDeudores(primaryStage, layout, mainPanel, eventHandler);
                } else if (frameActivo.contains("Deudores")) {
                    layout.getChildren().clear();
                    windowsResumen(primaryStage, layout, mainPanel, eventHandler);

                }
            } else if (event.getCode() == KeyCode.LEFT) {
                if (frameActivo.contains("Deudores")) {
                    layout.getChildren().clear();
                    windowsInforme(primaryStage, layout,mainPanel,eventHandler);
                } else if (frameActivo.contains("Resumen")) {
                    layout.getChildren().clear();
                    windowsDeudores(primaryStage, layout, mainPanel, eventHandler);
                }
            }
    ...
    public void windowsDeudores (Stage primaryStage, Pane layout, BorderPane mainPanel, EventHandler eventHandler) {
        ...
        Label changeFrameLeft = new Label("\uD83E\uDC98");
        changeFrameLeft.setFont(new Font("Cooper Black", 30));
        changeFrameLeft.setTextFill(Color.web("#FFFFFF"));
        changeFrameLeft.relocate(175,11);
        layout.getChildren().add(changeFrameLeft);

        Label changeFrameRight = new Label("\uD83E\uDC9A");
        changeFrameRight.setFont(new Font("Cooper Black", 30));
        changeFrameRight.setTextFill(Color.web("#FFFFFF"));
        changeFrameRight.relocate(277,11);
        layout.getChildren().add(changeFrameRight);
        ...
        Label labelTable = new Label("(TABLA Z)");
        labelTable.setFont(new Font("Cooper Black", 10));
        labelTable.setTextFill(Color.web("#FFFFFF"));
        labelTable.relocate(200, 185);
        layout.getChildren().add(labelTable);
        ...
        eventHandler = new EventHandler<javafx.scene.input.MouseEvent>() {
            @Override
            public void handle(MouseEvent e) {
                if (e.getTarget() == changeFrameLeft) {
                    if (e.getEventType() == MouseEvent.MOUSE_ENTERED) {
                        changeFrameLeft.setTextFill(Color.web("#000000"));
                    } else {
                        changeFrameLeft.setTextFill(Color.web("#FFFFFF"));
                    }
                }

                if (e.getTarget() == changeFrameRight) {
                    if (e.getEventType() == MouseEvent.MOUSE_ENTERED) {
                        changeFrameRight.setTextFill(Color.web("#000000"));
                    } else {
                        changeFrameRight.setTextFill(Color.web("#FFFFFF"));
                    }
                }
                ...
                if (e.getEventType() == MouseEvent.MOUSE_CLICKED) {
                    if (e.getSource() == changeFrameLeft) {
                        layout.getChildren().clear();
                        windowsInforme(primaryStage,layout,mainPanel, finalEventHandler);
                    } else if (e.getSource() == changeFrameRight) {
                        layout.getChildren().clear();
                        windowsResumen(primaryStage, layout, mainPanel, finalEventHandler);
                    }
        ...
        //interaction
        changeFrameLeft.addEventFilter(MouseEvent.MOUSE_ENTERED, eventHandler);
        changeFrameLeft.addEventFilter(MouseEvent.MOUSE_EXITED, eventHandler);
        changeFrameLeft.addEventFilter(MouseEvent.MOUSE_CLICKED, eventHandler);
        changeFrameRight.addEventFilter(MouseEvent.MOUSE_ENTERED, eventHandler);
        changeFrameRight.addEventFilter(MouseEvent.MOUSE_EXITED, eventHandler);
        changeFrameRight.addEventFilter(MouseEvent.MOUSE_CLICKED, eventHandler);

        mainPanel.setCenter(layout);
    }

    public void windowsResumen (Stage primaryStage, Pane layout, BorderPane mainPanel, EventHandler eventHandler) {

        Label labelDesc = new Label("Resumen");
        labelDesc.setFont(new Font("Cooper Black", 14));
        labelDesc.setTextFill(Color.web("#FFFFFF"));
        labelDesc.relocate(205,20);
        layout.getChildren().add(labelDesc);
        ...
        changeFrame.setFont(new Font("Cooper Black", 30));
        changeFrame.setTextFill(Color.web("#FFFFFF"));
        changeFrame.relocate(180,12);
        layout.getChildren().add(changeFrame);

        Label selectLabel = new Label("SELECCIONE EL HISTORICO");
        selectLabel.setFont(new Font("Cooper Black", 26));
        selectLabel.setTextFill(Color.web("#FFFFFF"));
        selectLabel.relocate(45, 150);
        layout.getChildren().add(selectLabel);
